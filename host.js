return {
  ...
  apply(ctx) {
    ...
    async function fetchAccount(account, signal) {
      ...
      try {
        ...
        if (result.exitCode !== 0) {
          ...
          return Object.assign({}, base, { ok: false, error: detail || 'curl 退出码 ' + result.exitCode, balances: [], low: [] })
        }
    ...
    harness.handle('get-usage', async (args) => {
    ...
    harness.handle('list-plugins', async () => {
      ...
      if (!modules) return { error: 'clientModules 服务不可用', total: 0, plugins: [] }
      try {
    ...
    harness.handle('open-plugin-dir', async (args) => {
      const id = args && typeof args.id === 'string' ? args.id : ''
      const modules = ctx.get('clientModules')
      if (!shell || !modules || !id) return { ok: false, error: '不可用' }
      let path = ''
      try { path = modules.clientPath(id) || '' } catch (e) { path = '' }
    ...
    harness.handle('get-state', async () => getStatePayload())
    harness.handle('refresh', async () => { await pollAll(); return getStatePayload() })
    harness.handle('recharge', async () => ({ label: 'DeepSeek 官方充值', url: RECHARGE_URL, usageUrl: USAGE_URL }))
