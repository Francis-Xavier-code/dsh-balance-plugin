...
`)
      ...
      if (!third.length) {
        ...
        const rows = third.map((plugin) => {
          const cells = [
            ...
            React.createElement('td', { key: 'op' }, React.createElement('button', { type: 'button', className: 'bmon-btn', disabled: !plugin.path, onClick: () => { host.call('open-plugin-dir', { id: plugin.id }).catch(() => {}) } }, '打开目录')),
          ]
    ...
    function DockReadout() {
      ...
      elements.push(React.createElement('button', { key: 'refresh', type: 'button', className: 'bmon-btn', title: '立即刷新', onClick: () => { host.call('refresh', {}).catch(() => {}) } }, '↻'))
      ...
      if (!snapshot) {
        ...
        if (draft) {
          ...
          configChildren.push(React.createElement('div', { key: 'actions', className: 'bmon-actions' },
            ...
            React.createElement('button', { type: 'button', className: 'bmon-btn', onClick: () => { host.call('refresh', {}).catch(() => {}) } }, '立即刷新'),
    ...
    function RunCardView() {
      const snapshot = useBalanceState()[0]
      const children = [
        React.createElement('div', { key: 'title', className: 'bmon-title' }, 'DeepSeek 余额监控'),
        React.createElement('div', { key: 'goto', className: 'bmon-hint' }, '完整配置请点输入框右侧的「钱包」图标打开面板。'),
      ]
      if (!snapshot) {
        children.push(React.createElement('div', { key: 'loading', className: 'bmon-hint' }, '正在连接 Host…'))
      } else {
        ...
        children.push(React.createElement('div', { key: 'actions', className: 'bmon-actions' },
          React.createElement('button', { type: 'button', className: 'bmon-btn', onClick: () => { host.call('refresh', {}).catch(() => {}) } }, '立即刷新'),
          React.createElement('a', { className: 'bmon-btn', href: snapshot.recharge.url, target: '_blank', rel: 'noreferrer' }, '去充值 ↗'),
          React.createElement('a', { className: 'bmon-btn', href: snapshot.recharge.usageUrl, target: '_blank', rel: 'noreferrer' }, '用量页 ↗'),
        ))
    ...
    function UsagePage() {
      ...
      children.push(React.createElement('div', { key: 'toolbar', className: 'bmon-u-toolbar' },
        ...
        React.createElement('button', { type: 'button', className: 'bmon-btn', onClick: () => { host.call('get-usage', { range: range }).then((value) => { if (value && typeof value === 'object') pair[1](value) }).catch(() => {}) } }, '↻'),
      ))
      if (!snapshot) {
        children.push(React.createElement('div', { key: 'loading', className: 'bmon-hint' }, '正在连接 Host…'))
        return React.createElement('div', { className: 'bmon-page' }, ...children)
    ...
    slots.inject('tool.view.cordis', () => slots.register(
      ...
      () => React.createElement(RunCardView),
    ))
  },
}
