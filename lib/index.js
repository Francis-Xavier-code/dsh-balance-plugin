...
export function apply(ctx) {
    ...
    try {
      ...
      const pluginPromises = entries.map(async (entry) => {
        ...
        if (!repository && !official && shell) {
          try {
            const cmd = 'npm view ' + String(entry.id || '') + ' repository.url 2>/dev/null'
            const spec = shell.resolve({ command: cmd, timeoutMs: 8000 })
            const result = await shell.run(spec)
            const url = (result.stdout.text || '').trim()
            if (url && result.exitCode === 0) repository = url
          } catch (e) { /* ignore */ }
        }
        return {
          id: String(entry.id || ''),
  ...
  registerRoute('update-plugin', async (args) => {
    ...
    try {
      ...
      const addCmd = 'UPDATE=1 dsh plugin --profile ' + profile + ' add ' + pkg
